                                    ...
                                    <td>
                                        <div class="d-flex align-items-center justify-content-between">
                                            ...
                                            <button type="button"
                                                ...
                                                data-toggle="modal"
                                                data-target="#setAssistantScheduleFormModal_{{ $psr->id }}">
                                                @if ($psr->schedules->count() > 0)
                                                    Ubah Jadwal
                                                @else
                                                    Tentukan Jadwal
                                            ...
                                            <div class="modal fade" id="setAssistantScheduleFormModal_{{ $psr->id }}"
                                                ...
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <form method="POST" action="{{ route('admin.assistant.schedule.store') }}">
                                                            @csrf
                                                                    ...
                                                                    @forelse ($schedules->where('classroom.period_subject.id', $psr->period_subject->id) as $schedule)
                                                                        <div class="d-flex align-items-center mb-2 border p-1 bg-white rounded-sm">
                                                                            <input type="checkbox" name="schedule[]"
                                                                                ...
                                                                                {{-- @isset($psr->schedule) @checked($psr->schedule->id == $schedule->id) @endisset --}}
                                                                                {{ in_array($schedule->id, $current_row_schedules) ? 'checked' : '' }}
                                                                                {{ 
                                                                            ...
                                                                            <label class="d-block m-0 w-100"
                                                                                ...
                                                                                <p class="d-flex m-0 mb-1">
                                                                                    <span
                                                                                        class="font-weight-normal w-25">Kelas
                                                                                    </span>
                                                                                ...
                                                                                <p class="d-flex m-0 mb-1">
                                                                                    <span
                                                                                        class="font-weight-normal w-25">Jadwal
                                                                                    </span>
                                                                                    <span>{{ $schedule->day }},
                                                                                        {{ $schedule->start_time }} -
                                                                                        {{ $schedule->end_time }}
                                                                                ...
                                                                                <p class="d-flex m-0 mb-1">
                                                                                    <span
                                                                                        class="font-weight-normal w-25">Ruangan
                                                                                    </span>
                                                                                ...
                                                                                <p class="d-flex m-0">
                                                                                    <span
                                                                                        class="font-weight-normal w-25">Asisten
                                                                                    </span>
                                                                                    <span>
                                                                                        {{ $schedule->psrs_count }}/{{ $schedule->number_of_lab_assistant }}
                                                                                    </span>
                                                                                ...
                                                                                @if ($schedule->psrs_count >= $schedule->number_of_lab_assistant)
                                                                                    @if (in_array($schedule->id, $current_row_schedules))
                                                                                        <small class="text-warning font-italic mt-1">
                                                                                            Menghapus centang akan mengeluarkan asisten praktikum ini dari jadwal. 
                                                                                        </small>
                                                                                    @else
                                                            ...
                                                            <div class="modal-footer">
                                                                <button type="submit" class="btn {{ $psr->schedules->count() > 0 ? 'btn-primary' : 'btn-success' }}">
                                                                    @if ($psr->schedules->count() > 0)
                                                                        SIMPAN PERUBAHAN
                                                                    @else
                                                                        SIMPAN JADWAL
