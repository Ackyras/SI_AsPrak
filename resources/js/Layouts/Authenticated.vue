<template>
    <div
        :class="{
            'bg-slate-50': $page.url.startsWith('user/ujian-seleksi'),
        }"
    >
        <sidebar />
        <div class="relative md:ml-64 bg-blueGray-100">
            ...
            <div>
                <div class="w-full px-4 mx-auto md:px-10 md:pt-16">
                    <!-- WARNING MESSAGE IF NOT PASS EXAM SELECTION YET -->
                    <div
                        class=""
                        v-if="show_exam_warning && $page.url === '/dashboard'"
                    >
                        <div
                            class="flex items-start justify-between p-2 my-4 rounded bg-amber-50 drop-shadow"
                        >
                            <div
                                class="flex items-center gap-2 w-10/11 md:w-11/12"
                            >
                                ...
                                <p
                                    class="text-sm font-medium text-gray-600 md:text-base"
                                >
                                    Saat ini anda login sebagai
                                    <span class="font-semibold"
                                        >Calon Asisten Praktikum</span
                                    >. Segera selesaikan
                                    <span>
                                        <Link
                                            :href="'/#'"
                                            class="font-semibold text-emerald-600 hover:text-amber-500"
                                        >
                                            Ujian Seleksi
                                        </Link>
                                    </span>
                    ...
                    <message-alert
                        ...
                        :msg-data="$page.props.flash.alert"
                    />
                    <!-- <div v-if="$page.props.alert"
                        class="flex items-center justify-between p-2 my-4 font-semibold text-white rounded drop-shadow" :class="{
                            'bg-emerald-500': $page.props.alert.status == 'success',
                            'bg-red-500': $page.props.alert.status == 'failed',
                            'hidden': !show_message,
                        }">
                        <div class="text-sm w-10/11">
                            <p>
