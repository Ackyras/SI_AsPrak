        ...
        <div class="min-h-[100vh-8.25rem-2px] md:min-h-[100vh-8.75rem]">
            ...
            <div class="mb-4 w-full md:w-3/4 lg:w-2/3 xl:1/2 mx-auto">
                <form @submit.prevent="submit">
                    ...
                    <div class="bg-white rounded drop-shadow p-3 md:p-4 mb-4" v-for="(question, index) in db_questions" :key="index">
                        <!-- ESSAY -->
                        <div v-if="question.type === 'essay'">
                            <p class="mb-3">
                                {{ question.text }}
                            </p>
                            <input type="file" :name="'answer_'+index" :id="'answer_'+index" @input="form.answers[index] = $event.target.files[0]" required
                            ...
                            <p class="mt-1 text-xs text-gray-500" id="file_input_help">JPG, JPEG, PNG, atau PDF (MAX. 2MB).</p>
                        </div>
                        <!-- PILIHAN BERGANDA -->
                        <card-question v-if="question.type === 'pilihan berganda'" :question-data="question" v-model="form.answers[index]" />
                    </div>
