<template>
    <div class="bg-white rounded drop-shadow pt-3">
        {{ getStatus(subjectData.period_subject.exam_end, subjectData.is_take_exam_selection) }}
        <div class="flex justify-between items-center mb-3 px-3">
            ...
            <div>
                ...
                <div class="flex gap-2 items-center">
                    ...
                    <p class="text-sm font-semibold text-gray-500">{{ toDateTimeFormat(subjectData.period_subject.exam_end) }}</p>
                </div>
            </div>
            <!-- 'EXAM STATUS' INDICATOR -->
            <span class="rounded-full bg-red-400 p-[10px] text-white"
                    v-bind:class="{
                        'block'      : isPastTime,
                        'hidden'     : !isPastTime,
                    }" 
            ...
            <span class="rounded-full bg-yellow-300 p-[10px] text-white"
                    v-bind:class="{
                        'block'      : isWaiting,
                        'hidden'     : !isWaiting,
                    }" 
            ...
            <span class="rounded-full bg-emerald-500 p-[10px] text-white"
                    v-bind:class="{
                        'block'      : isDone,
                        'hidden'     : !isDone,
                    }" 
        ...
        <Link class="flex gap-2 w-full py-2 px-3 border-t-[1px] items-center text-xs text-yellow-500 hover:bg-yellow-50 hover:text-yellow-600 font-thin rounded-b"
            v-bind:class="{
                'flex'    : isWaiting,
                'hidden'  : !isWaiting,
            }" 
